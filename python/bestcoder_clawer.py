...
from config import*
#获取poj网页     
def getHtml_bestcoder(url):  
    page = urllib.urlopen(url)  
    html = page.read()  
    return html  

#获取poj中用户信息  
def zhenghe_bestcoder(str1,userid,imgre):  
    html =getHtml_bestcoder( str1+ userid) 
    return re.findall(imgre,html)  

def output_bestcoder(result_bestcoder):  #result_xoj: xoj用户数据界面的前缀url
    ojname_list = [] #oj帐号列表
    reg_solved = '<td>(.+?) / .+?:.+?:.+?</td>'
    imgre_solved = re.compile(reg_solved)  

    reg_recentProblem = 'cid=[0-9]+?">(.+?)</a>'
    imgre_recentProblem = re.compile(reg_recentProblem)
    
    reg_recentID = 'cid=([0-9]+?)"'  #获取cf的比赛号，生成超链接时使用
    ...
    # SQL 查询语句 
    sql = "SELECT distinct ojusername FROM clawer  WHERE  ojname='bestcoder' " 
    try:
      # 执行SQL语句
      cursor.execute(sql)
      # 获取所有记录列表
    ...
    for line in ojname_list:  #每一行为一个用户名，分别分析
        line=line.strip('\n')    #去掉读取的每行的"\n"  
        list_solved = zhenghe_bestcoder(result_bestcoder,line,imgre_solved)
        number_solved=0   
        ...
        list_rating = zhenghe_bestcoder(result_bestcoder,line,imgre_rating)  #爬取rating 数据
        ...
        list_recentProblem= zhenghe_bestcoder(result_bestcoder,line,imgre_recentProblem)
